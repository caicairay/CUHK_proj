...
##_____________PreWork______________#
    ...
    mesh=rhos>threshold                                                               #
    mesh=np.array([ii for (ii,boo) in np.ndenumerate(mesh) if boo]).astype(float)     #
    mesh=mesh[::2]                                                                   #Generate start points
    #mesh=np.random.rand(3,2)*240
...
##______________SCMSLOOP_______________##
    ...
    while np.max(error)>=eps and t<maxt:
        ...
        for (m,pos) in enumerate(mesh[:,]):
            ...
            error[m]=np.sqrt(np.sum(pjstep**2)/np.sum(shift**2))
            npos=np.array([x+y for x,y in zip(pos,pjshift)])
            npos=npos.clip(min=0,max=rhos.shape[0]-1).reshape([2,])
            mesh[m,]=npos
            print mesh[m,]
        if plot_switch==True:
            plot(t,mesh,rhos,plot_path)
...
##_____________DoubleCheck______________##
    ...
    for (m,pos) in enumerate(mesh):
        round_pos=np.array([int(round(ii)) for ii in pos])
        if w0[m] or rhos[tuple(round_pos)]<threshold:
            mesh[m,]=None
